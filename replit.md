...
- **Loyalty Points** (`/dashboard/loyalty`): Customers earn points on every purchase (configurable pts per ₹100). Points redeemable at checkout as discount (configurable ₹ value per point, min points threshold, max % of order). Toggle in checkout just like wallet. Points history page with golden balance card. Admin panel (`/dashboard/admin-loyalty`) to view all members' balances and manually adjust points (add bonus or deduct). Admin configures rates in Site Settings (Loyalty Points section). `LoyaltyModel` stores per-user balances + transaction history.
- **Site Settings** (`/dashboard/site-settings`): Theme color, policy pages, store name/logo, social links, WhatsApp button, return period, **Maintenance Mode** (on/off + custom message), **COD Restriction** (enable/disable cash on delivery), **Loyalty Points Program** (earn rate, point value, min/max redeem)
