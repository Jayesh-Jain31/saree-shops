  ...
  return (
    <div>
        ...
        <div className='text-sm grid gap-1'>
            ...
            {isAdmin(user.role) && (
              <Link onClick={handleClose} to={"/dashboard/subcategory"} className='px-2 hover:bg-orange-200 py-1 rounded'>Sub Category</Link>
            )}
            {isAdmin(user.role) && (
              <Link onClick={handleClose} to={"/dashboard/upload-product"} className='px-2 hover:bg-orange-200 py-1 rounded'>Upload Product</Link>
            )}
            {isAdmin(user.role) && (
              <Link onClick={handleClose} to={"/dashboard/product"} className='px-2 hover:bg-orange-200 py-1 rounded'>Product</Link>
            )}
            {isAdmin(user.role) && (
              <Link onClick={handleClose} to={"/dashboard/coupons"} className='px-2 hover:bg-orange-200 py-1 rounded'>Coupons</Link>
            )}
