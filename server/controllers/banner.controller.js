...
export async function createBannerController(request, response) {
    try {
        const { title, slides, image, imageMobile, link, isActive, displayOrder, placement } = request.body
        ...
        const count = await BannerModel.countDocuments({ placement: placement || 'hero' })
...
export async function updateBannerController(request, response) {
    try {
        ...
        const banner = await BannerModel.findByIdAndUpdate(
            id,
            { title, slides, image, imageMobile, link, isActive, displayOrder, placement },
            { new: true }
        )
        if (!banner) {
