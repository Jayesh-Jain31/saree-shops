...
export async function createCouponController(request, response) {
    try {
        const { code, discountType, discountValue, minOrderAmount, maxDiscount, isActive, expiresAt, usageLimit, razorpayOfferId } = request.body
        ...
        const coupon = new CouponModel({
            ...
            razorpayOfferId: razorpayOfferId ? razorpayOfferId.trim() : null,
        })
...
export async function updateCouponController(request, response) {
    try {
        const { _id, code, discountType, discountValue, minOrderAmount, maxDiscount, isActive, expiresAt, usageLimit, razorpayOfferId } = request.body
        ...
        if (razorpayOfferId !== undefined) updateData.razorpayOfferId = razorpayOfferId ? razorpayOfferId.trim() : null
