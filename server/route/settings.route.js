...
import { sendOrderConfirmationWhatsApp, sendCODVerificationWhatsApp } from '../utils/whatsapp.js'
...
settingsRouter.post('/test-whatsapp', auth, admin, async (req, res) => {
    try {
        ...
        return res.json({ message: `Test WhatsApp sent via '${template}' template to ${mobile}. Check Railway logs for result.`, error: false, success: true })
    } catch (err) {
